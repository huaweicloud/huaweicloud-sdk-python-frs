...
class FaceServiceV2(object):
    ...
    def _add_face(self, face_set_name, image, image_type, external_image_id, external_fields):
        ...
        :rtype: AddFaceResult
        """
        uri = FrsConstantV2.FACE_ADD_URI % (self.project_id, face_set_name)
        request_body = {}
    ...
    def _get_face(self, face_set_name, offset=None, limit=None, face_id=None):
        ...
        """
        if face_id:
            uri = FrsConstantV2.FACE_GET_ONE_URI % (self.project_id, face_set_name, face_id)
        elif offset or limit:
            uri = FrsConstantV2.FACE_GET_RANGE_URI % (self.project_id, face_set_name, offset, limit)
        else:
            uri = FrsConstantV2.FACE_GET_BASE_URI % (self.project_id, face_set_name)
    ...
    def _delete_face(self, face_set_name, external_image_id=None, face_id=None, field_id=None, field_value=None):
        ...
        """
        if external_image_id:
            uri = FrsConstantV2.FACE_DELETE_BY_EXTERNAL_IMAGE_ID_URI % (self.project_id, face_set_name, external_image_id)
        elif face_id:
            uri = FrsConstantV2.FACE_DELETE_BY_FACE_ID_URI % (self.project_id, face_set_name, face_id)
        else:
            uri = FrsConstantV2.FACE_DELETE_BY_FIELD_ID_URI % (self.project_id, face_set_name, field_id, field_value)
